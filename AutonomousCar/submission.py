...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for i in range(self.belief.numRows):
            for j in range(self.belief.numCols):
                distance = math.sqrt((agentX - util.colToX(j)) ** 2 + (agentY - util.rowToY(i)) ** 2)
                probability = util.pdf(distance, Const.SONAR_STD, observedDist)
                prior_prob = self.belief.getProb(i, j)
                self.belief.setProb(i, j, prior_prob * probability)
    ...
    def elapseTime(self):
        ...
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        my_belief = util.Belief(self.belief.numRows, self.belief.numCols, 0)
        for old, new in self.transProb:
            prior_prob = self.belief.getProb(old[0], old[1])
            transition_prob = self.transProb[(old, new)]
            my_belief.addProb(new[0], new[1], prior_prob * transition_prob)
        my_belief.normalize()
        self.belief = my_belief
        # END_YOUR_CODE
